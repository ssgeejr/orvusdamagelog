...
#    cur.execute("select sum(amount) as totaldamage,source,target from orbus where action = 'COMBAT' group by source order by totaldamage desc");
	...
	boss1_total_damage = 0
	boss2_total_damage = 0


    print("**** BOSS-1 ****")
    cur.execute("select max(time) as mx, min(time) as mn from orbus where target like ? and action = 'COMBAT' and time like ? " , (boss1, boss1_time));
    ...
    print("****RAW DAMAGE****")
    cur.execute("select time,amount,target,source from orbus where target like ? and action = 'COMBAT'  and time like ? order by time" , (boss1, boss1_time)););
    for row in cur:
        print(row)


    print("****TOTAL DAMAGE****")
    cur.execute("select sum(amount) as totaldamage,source,target from orbus where target like ? and action = 'COMBAT'  and time like ? group by source,target order by target" , (boss1, boss1_time)););
    for row in cur:
        print("Raw Damage [%s], DPS [%s], Player [%s]" % (row[0], dps(row[0],cmbseconds),row[1]))
		boss1_total_damage+=row[0]

	print("**** BOSS-2 ****")
    cur.execute("select max(time) as mx, min(time) as mn from orbus where target like ? and action = 'COMBAT' and time like ? " , (boss2, boss2_time));
    ...
    print("****RAW DAMAGE****")
    cur.execute("select time,amount,target,source from orbus where target like ? and action = 'COMBAT'  and time like ? order by time" , (boss2, boss2_time)););
    for row in cur:
        print(row)


    print("****TOTAL DAMAGE****")
    cur.execute("select sum(amount) as totaldamage,source,target from orbus where target like ? and action = 'COMBAT'  and time like ? group by source,target order by target" , (boss2, boss2_time)););
    for row in cur:
        print("Raw Damage [%s], DPS [%s], Player [%s]" % (row[0], dps(row[0],cmbseconds),row[1]))
		boss2_total_damage+=row[0]


		
		
	print("TOTAL DAMAGE BOSS-1")
	print("TOTAL DAMAGE BOSS-1: %s" % boss1_total_damage)
	print("TOTAL DAMAGE BOSS-2")
	print("TOTAL DAMAGE BOSS-2: %s" % boss2_total_damage)
	print("TOTAL DAMAGE BOSS-2")
	print("TOTAL DAMAGE BOSS-1 & BOSS-2: %s" % (boss1_total_damage + boss2_total_damage) )
